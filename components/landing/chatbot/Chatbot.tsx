...
import { X, User, SendHorizonal, Loader2, SendHorizontal } from 'lucide-react'
...
export default function Chatbot({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const skipTurnstileVerification = !turnstileSiteKey && process.env.NODE_ENV === 'development'
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (normalizedResp === 'LIMITREACHED') {
      ...
      } else if (normalizedResp.toLowerCase() === 'flag' || /"flag"/i.test(normalizedResp)) {
        setEscalation('flag')
        await streamBotResponse(
        ...
        await streamBotResponse(responseText || "Sorry, I couldn't get a response right now.", {
          showFaqOnComplete: false,
        })
      }
    } catch (err) {
      const errMessage = (err as Error)?.message || 'Failed to fetch response. Please try again.'
      setMessages(prev => [
        ...prev,
        {
              ...
              >
                {message.sender === 'bot' && (
                  <div className="w-8 h-8 rounded-full bg-[#E9EAEB] flex items-center justify-center flex-shrink-0">
                    <Image src="/botLogo.svg" alt="Chatbot" width={16} height={16} />
                  </div>
                )}
                {message.sender === 'user' && (
          ...
          {!token && (
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-lg p-4">
                <h3 className="font-semibold text-gray-800 mb-2">Human Verification</h3>
                ...
                {turnstileSiteKey ? (
                  ...
                  <p className="text-sm text-amber-700">
                    Chat verification is not configured. Set NEXT_PUBLIC_CLOUDFLARE_SITE_KEY to
          ...
          {showFAQ && isEmailSubmitted && !escalation && (
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  onClick={() =>
                    handleFAQClick('What makes Karsaaz QR different from other QR generators?')
                  }
                  className="p-2 text-xs border border-gray-300 rounded-lg hover:bg-gray-100 transition-colors text-left text-black cursor-pointer"
                >
                  What makes Karsaaz QR different from other QR generators?
                ...
                <button
                  onClick={() =>
                    handleFAQClick('What pricing plans and benefits does Karsaaz QR offer?')
                  }
                  className="p-2 text-xs border border-gray-300 rounded-lg hover:bg-gray-100 transition-colors text-left text-black cursor-pointer"
                >
                  What pricing plans and benefits does Karsaaz QR offer?
