...
export function ReferralCodeShare() {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      <h3 className="text-lg font-semibold text-gray-900">{t('Your Referral Link')}</h3>
      <p className="mt-1 text-sm text-gray-600">
        {t("Share this link with others. You'll earn a commission when they sign up.")}
      </p>
      ...
      {code && (
        <p className="mt-3 text-xs text-gray-500">
          {t('Referral code:')} <span className="font-mono font-medium text-gray-700">{code}</span>
        </p>
      )}
    </div>
  )
