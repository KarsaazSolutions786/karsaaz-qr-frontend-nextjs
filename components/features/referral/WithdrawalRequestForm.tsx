...
export function WithdrawalRequestForm({ onSuccess }: WithdrawalRequestFormProps) {
  ...
  const onSubmit = async (data: WithdrawalFormData) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : t('Failed to submit withdrawal request')
      setError(message)
    } finally {
      setSubmitting(false)
    }
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
      <h3 className="text-lg font-semibold text-gray-900">{t('Request Withdrawal')}</h3>
      <p className="mt-1 text-sm text-gray-600">
        {t('Submit a request to withdraw your available commission balance.')}
      </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            placeholder={t('Enter your payment details (e.g., bank account number, PayPal email, wallet address)')}
          />
