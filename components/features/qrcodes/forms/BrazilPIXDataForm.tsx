...
export function BrazilPIXDataForm({ defaultValues, onChange }: BrazilPIXDataFormProps) {
  ...
  return (
    <form className="space-y-5">
      <div>
        <label htmlFor="key" className={LABEL}>{t('PIX Key')}</label>
        <input {...register('key')} id="key" type="text" placeholder={t('CPF, CNPJ, phone, email or random key')} className={INPUT} />
        {errors.key && <p className={ERROR}>{errors.key.message}</p>}
      </div>
      <div>
        <label htmlFor="name" className={LABEL}>{t('Recipient Name')}</label>
      ...
      <div>
        <label htmlFor="amount" className={LABEL}>{t('Amount')} <span className="text-gray-400 font-normal">({t('optional')})</span></label>
        <input {...register('amount', { valueAsNumber: true })} id="amount" type="number" step="0.01" placeholder="0.00" className={INPUT} />
        {errors.amount && <p className={ERROR}>{errors.amount.message}</p>}
      </div>
      <div>
        <label htmlFor="transaction_id" className={LABEL}>{t('Transaction ID')} <span className="text-gray-400 font-normal">({t('optional')})</span></label>
      ...
      <div>
        <label htmlFor="message" className={LABEL}>{t('Message')} <span className="text-gray-400 font-normal">({t('optional')})</span></label>
