...
/**
 * Purpose: Executes FileUploadDataForm functionality.
...
export function FileUploadDataForm({ defaultValues, onChange }: FileUploadDataFormProps) {
  ...
  return (
    <form className="space-y-5">
      <div>
        <label htmlFor="name" className={LABEL}>{t('File Name')}</label>
        <input {...register('name')} id="name" type="text" placeholder={t('e.g. My Document')} className={INPUT} />
        {errors.name && <p className={ERROR}>{errors.name.message}</p>}
      </div>
      <div className="p-4 bg-purple-50 rounded-xl border border-purple-200 text-sm text-purple-700">
        {t('File uploads are managed through the backend. Set the file name here, then upload your file in the QR code details.')}
      </div>
      <div>
