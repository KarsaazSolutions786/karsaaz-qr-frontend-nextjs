...
interface ProductCatalogueDataFormProps { defaultValues?: Partial<ProductCatalogueData>; onChange?: (data: Partial<ProductCatalogueData>) => void }
export function ProductCatalogueDataForm({ defaultValues, onChange }: ProductCatalogueDataFormProps) {
  const { register, formState: { errors } } = useQRFormWatch<ProductCatalogueData>({ schema: productCatalogueDataSchema, defaultValues, onChange })
  return (
    <form className="space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          <label htmlFor="business_name" className={LABEL}>Business Name *</label>
          <input {...register('business_name')} id="business_name" type="text" placeholder="My Store" className={INPUT} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          ...
          <input {...register('email')} id="email" type="email" placeholder="info@store.com" className={INPUT} />
          {errors.email && <p className={ERROR}>{errors.email.message}</p>}
        </div>
        <div>
          <label htmlFor="website" className={LABEL}>Website</label>
          <input {...register('website')} id="website" type="url" placeholder="https://mystore.com" className={INPUT} />
