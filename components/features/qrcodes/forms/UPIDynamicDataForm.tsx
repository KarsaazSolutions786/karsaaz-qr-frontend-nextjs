...
export function UPIDynamicDataForm({ defaultValues, onChange }: UPIDynamicDataFormProps) {
  ...
  return (
    <form className="space-y-5">
      <div>
        <label htmlFor="payee_name" className={LABEL}>{t('Payee Name')}</label>
      ...
      <div>
        <label htmlFor="upi_id" className={LABEL}>{t('UPI ID')}</label>
      ...
      <div>
        <label htmlFor="expires_at" className={LABEL}>{t('Expires At')} <span className="text-gray-400 font-normal">({t('optional')})</span></label>
