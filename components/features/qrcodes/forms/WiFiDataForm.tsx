...
export function WiFiDataForm({ defaultValues, onChange }: WiFiDataFormProps) {
  ...
  const {
    register,
    formState: { errors },
  } = useQRFormWatch<WiFiDataFormData>({ schema: wifiDataSchema, defaultValues, onChange })
  return (
    <form className="space-y-5">
      ...
      <div>
        <label htmlFor="password" className={LABEL}>
          {t('Password')} <span className="text-gray-400 font-normal">({t('optional')})</span>
        </label>
        <input
          {...register('password')}
          id="password"
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        <label htmlFor="hidden" className="text-sm font-medium text-gray-700 cursor-pointer">
          {t('Hidden network (SSID not broadcast)')}
        </label>
      </div>
    </form>
  )
