...
export function WiFiDataForm({ defaultValues, onChange }: WiFiDataFormProps) {
  ...
  return (
    <form className="space-y-5">
      <div>
        <label htmlFor="ssid" className={LABEL}>{t('Network Name (SSID)')}</label>
        <input {...register('ssid')} id="ssid" type="text" placeholder={t('My WiFi Network')} className={INPUT} />
        {errors.ssid && <p className={ERROR}>{errors.ssid.message}</p>}
      </div>
      <div>
        <label htmlFor="password" className={LABEL}>{t('Password')} <span className="text-gray-400 font-normal">({t('optional')})</span></label>
        <input {...register('password')} id="password" type="text" placeholder={t('Leave blank for open networks')} className={INPUT} />
      </div>
      <div>
        <label htmlFor="encryption" className={LABEL}>{t('Encryption Type')}</label>
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        <label htmlFor="hidden" className="text-sm font-medium text-gray-700 cursor-pointer">{t('Hidden network (SSID not broadcast)')}</label>
      </div>
    </form>
  )
}
