...
/**
 * Purpose: Executes VCardPlusDataForm functionality.
    ...
    <form className="space-y-5">
      ...
      <fieldset className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            <label htmlFor="firstName" className={LABEL}>{t('First Name')} *</label>
            <input {...register('firstName')} id="firstName" type="text" placeholder="John" className={INPUT} />
            {errors.firstName && <p className={ERROR}>{errors.firstName.message}</p>}
          </div>
          <div>
            <label htmlFor="lastName" className={LABEL}>{t('Last Name')}</label>
            <input {...register('lastName')} id="lastName" type="text" placeholder="Doe" className={INPUT} />
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            <label htmlFor="phones" className={LABEL}>{t('Phone(s)')}</label>
            <input {...register('phones')} id="phones" type="text" placeholder="+1 234 567 890" className={INPUT} />
          </div>
          <div>
            <label htmlFor="emails" className={LABEL}>{t('Email(s)')}</label>
            <input {...register('emails')} id="emails" type="text" placeholder="john@example.com" className={INPUT} />
          </div>
        </div>
        <div>
          <label htmlFor="whatsapp_number" className={LABEL}>{t('WhatsApp Number')}</label>
          <input {...register('whatsapp_number')} id="whatsapp_number" type="text" placeholder={t('Include country code, e.g., +1234567890')} className={INPUT} />
        </div>
      </fieldset>

      {/* Job Details */}
      <fieldset className="space-y-4">
        <legend className="text-base font-bold text-gray-900">{t('Job Details')}</legend>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
          ...
          <div>
            <label htmlFor="job" className={LABEL}>{t('Job Title')}</label>
            <input {...register('job')} id="job" type="text" placeholder={t('Software Engineer')} className={INPUT} />
          </div>
        </div>
        <div>
          <label htmlFor="bio" className={LABEL}>{t('Bio')}</label>
          <textarea {...register('bio')} id="bio" placeholder={t('Short biography...')} className={TEXTAREA} />
        </div>
        <div>
      ...
      {/* Address */}
      <fieldset className="space-y-4">
        <legend className="text-base font-bold text-gray-900">{t('Address')}</legend>
        <div>
          <label htmlFor="street" className={LABEL}>{t('Street')}</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <div>
            <label htmlFor="state" className={LABEL}>{t('State')}</label>
            <input {...register('state')} id="state" type="text" placeholder="NY" className={INPUT} />
          </div>
          <div>
            <label htmlFor="zip" className={LABEL}>{t('ZIP Code')}</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            ...
            <input {...register('country')} id="country" type="text" placeholder={t('United States')} className={INPUT} />
          </div>
          <div>
            <label htmlFor="maps_url" className={LABEL}>{t('Maps URL')}</label>
            <input {...register('maps_url')} id="maps_url" type="text" placeholder={t('Google Maps link')} className={INPUT} />
          </div>
        </div>
      </fieldset>
      ...
      <div>
        <label htmlFor="openingHoursEnabled" className={LABEL}>{t('Opening Hours')}</label>
