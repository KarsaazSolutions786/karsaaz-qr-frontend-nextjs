...
/**
 * Purpose: Executes WebsiteBuilderDataForm functionality.
...
export function WebsiteBuilderDataForm({ defaultValues, onChange }: WebsiteBuilderDataFormProps) {
  ...
  return (
    <form className="space-y-5">
      <div>
        <label htmlFor="website_name" className={LABEL}>{t('Website Name')} *</label>
        <input {...register('website_name')} id="website_name" type="text" placeholder={t('My new website')} className={INPUT} />
      ...
      <div className="p-4 bg-purple-50 rounded-xl border border-purple-200 text-sm text-purple-700">
        {t('After creating the QR code, you can build out your website pages with the full website builder editor.')}
      </div>
    </form>
  )
}
