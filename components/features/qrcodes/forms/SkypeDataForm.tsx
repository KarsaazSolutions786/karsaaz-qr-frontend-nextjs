...
/**
 * Purpose: Executes SkypeDataForm functionality.
...
export function SkypeDataForm({ defaultValues, onChange }: SkypeDataFormProps) {
  ...
  return (
    <form className="space-y-5">
      <div>
        <label htmlFor="skype_name" className={LABEL}>{t('Skype Name / ID')}</label>
        <input {...register('skype_name')} id="skype_name" type="text" placeholder={t('Your Skype username')} className={INPUT} />
        {errors.skype_name && <p className={ERROR}>{errors.skype_name.message}</p>}
      </div>
      <div>
        <label htmlFor="type" className={LABEL}>{t('Action')}</label>
