...
/**
 * Purpose: Executes WeChatDataForm functionality.
...
export function WeChatDataForm({ defaultValues, onChange }: WeChatDataFormProps) {
  ...
  return (
    <form className="space-y-5">
      <div>
        <label htmlFor="username" className={LABEL}>{t('WeChat ID or Username')}</label>
        <input {...register('username')} id="username" type="text" placeholder={t('Your WeChat ID')} className={INPUT} />
