...
export const PROCESSOR_REGISTRY: Record<string, LazyProcessorComponent> = {
  ...
  flutterwave: lazy(() =>
    import('./FlutterwaveForm').then((m) => ({ default: m.FlutterwaveForm }))
  ),
  mercadopago: lazy(() =>
  ...
  '2checkout': lazy(() =>
    import('./TwoCheckoutForm').then((m) => ({ default: m.TwoCheckoutForm }))
  ),
  ...
  'payu-latam': lazy(() =>
    import('./PayULatamForm').then((m) => ({ default: m.PayULatamForm }))
  ),
  ...
  postfinance: lazy(() =>
    import('./PostFinanceForm').then((m) => ({ default: m.PostFinanceForm }))
  ),
  'orange-billing': lazy(() =>
