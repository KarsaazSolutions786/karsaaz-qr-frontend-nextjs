...
export default function TableBlock({ block, isEditing, onUpdate }: TableBlockProps) {
    ...
    return (
      <div className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
        ...
        <div className="flex items-center gap-2">
          <input
            type="checkbox"
            id={`bordered-${block.id}`}
            checked={bordered}
            onChange={e => onUpdate?.({ ...block.data, bordered: e.target.checked })}
  ...
  const cellBorderClass = bordered ? 'border border-gray-100' : ''

  return (
    <div className={`overflow-x-auto rounded-lg ${borderClass}`}>
      <table className="w-full text-sm" style={{ color: textColor || undefined }}>
        <thead className="bg-gray-50">
          <tr>
            {header.map((cell, i) => (
              <th
                key={i}
                className={`px-4 py-2 text-left font-semibold text-gray-900 ${cellBorderClass}`}
              >
                {cell}
              </th>
            ))}
        ...
        <tbody className={bordered ? '' : 'divide-y divide-gray-100'}>
          {body.map((row, rowIndex) => (
            <tr key={rowIndex}>
              {row.map((cell, cellIndex) => (
                <td key={cellIndex} className={`px-4 py-2 ${cellBorderClass}`}>
                  {cell}
                </td>
              ))}
            </tr>
