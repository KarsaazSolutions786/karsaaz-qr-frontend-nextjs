...
export async function exportPDF(svg: string, options: PDFExportOptions = {}): Promise<void> {
  const {
    ...
    metadata,
    compression = true,
    quality: _quality = 0.95, // eslint-disable-line @typescript-eslint/no-unused-vars
  } = options
  ...
  // Create PDF document
  const pdf = createPDFDocument(JsPDF, pageSize, orientation, unit, customWidth, customHeight)
...
function createPDFDocument(
  ...
  const format =
    pageSize === 'custom' && customWidth && customHeight ? [customWidth, customHeight] : pageSize
...
export function getPageSizeDimensions(
  ...
  return orientation === 'landscape' ? { width: dims.height, height: dims.width } : dims
}
