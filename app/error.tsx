...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 px-4">
      ...
      <h2 className="mt-4 text-2xl font-semibold text-gray-800 dark:text-gray-200">{t('Something went wrong')}</h2>
      <p className="mt-2 text-gray-500 dark:text-gray-400">{error.message || t('An unexpected error occurred.')}</p>
