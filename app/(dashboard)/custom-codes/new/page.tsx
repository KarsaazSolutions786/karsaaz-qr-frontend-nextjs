...
/**
 * Purpose: Executes NewCustomCodePage functionality.
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{t('New Custom Code')}</h1>
          <p className="mt-2 text-sm text-gray-600">{t('Inject custom scripts or styles into your application')}</p>
        </div>
        <Link href="/custom-codes" className="text-sm text-blue-600 hover:text-blue-900">{t('← Back')}</Link>
      </div>

      {createMutation.error && (
        <div className="mb-4 rounded-md bg-red-50 p-4 text-sm text-red-700">{t('Failed to create custom code. Please check the fields and try again.')}</div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <div>
          ...
          <input type="text" required value={form.name} onChange={(e) => set('name', e.target.value)}
            placeholder={t('e.g. Facebook Pixel')}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">{t('Language')} <span className="text-red-500">*</span></label>
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">{t('Lower numbers load first within the same position')}</p>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700">{t('Code')} <span className="text-red-500">*</span></label>
        ...
        <div className="flex items-center justify-end gap-4 border-t pt-4">
          ...
          <button type="submit" disabled={createMutation.isPending}
            ...
            {createMutation.isPending ? t('Creating…') : t('Create Custom Code')}
          </button>
        </div>
      </form>
    </div>
