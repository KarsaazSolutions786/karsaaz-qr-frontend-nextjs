...
const PROCESSORS: ProcessorDef[] = [
  ...
  {
    id: 'payu-international',
    slug: 'payu-international',
    name: 'PayU International',
    hasTestCredentials: true,
    fields: [
      {
        key: 'payu-international_mode',
        label: 'Mode',
        type: 'select',
        options: [
          { value: 'sandbox', label: 'Sandbox' },
      ...
      {
        key: 'payu-international_second_key',
        label: 'Second Key (MD5)',
        type: 'password',
        placeholder: '05dbc4a507c0e256...',
      },
      {
        key: 'payu-international_client_id',
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: 'alipay-china_app_auth_token',
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: 'paykickstart_secret_key',
        label: 'Secret Key',
        type: 'password',
        placeholder: 'Secret key',
      },
      {
        key: 'paykickstart_email_template',
        label: 'New Registration Email Template',
        type: 'textarea',
      },
      {
        key: 'paykickstart_upgrade_email_template',
        label: 'Upsale Email Template',
        type: 'textarea',
      },
    ],
  },
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: 'postfinance_tax_percentage',
  ...
  },
  {
    ...
    fields: [
      {
        key: 'offline-payment_customer_instructions',
        label: 'Customer Instructions (Markdown)',
        type: 'textarea',
        placeholder: 'Explain how customers should make offline payments…',
      },
      {
        key: 'offline-payment_payment_proof',
...
function ProcessorForm({ processor }: { processor: ProcessorDef }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
      ...
      {testResult && (
        <div
          className={`rounded-md p-4 text-sm ${testResult.startsWith('✓') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}
        >
          {testResult}
        </div>
      )}
          ...
          <div className="flex items-center gap-2 mt-2">
            <code className="rounded bg-white px-2 py-1 text-xs break-all select-all">
              {typeof window !== 'undefined'
                ? `${window.location.origin}/api/webhooks/${processor.slug}`
                : `/api/webhooks/${processor.slug}`}
            </code>
            <button
              type="button"
              onClick={() => {
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-4">
            {(['enabled', 'disabled'] as const).map(v => (
              <label
                key={v}
                className="flex cursor-pointer items-center gap-2 rounded-md border border-gray-200 px-4 py-2 text-sm font-medium hover:bg-gray-50"
              >
                <input
                  type="radio"
                  name={enabledKey}
        ...
        {CustomForm ? (
          <Suspense fallback={<div className="py-2 text-sm text-gray-400">Loading form…</div>}>
            <CustomForm settings={values} onChange={(key: string, val: string) => set(key, val)} />
          </Suspense>
        ) : (
