    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
        ...
        <p className="mb-4 text-sm text-gray-600">
          {t('Copy all content blocks from one language to another.')}
        </p>
...
function ContentBlocksPageInner() {
  ...
  const handleDeleteAll = async () => {
    ...
    const langName =
      allTranslations.find(tr => tr.id === translationId)?.name ?? `ID ${translationId}`
    if (
      !(await confirm({
        title: 'Are you sure?',
        message: t(
          'Delete ALL content blocks for language "{{name}}"? This cannot be undone.'
        ).replace('{{name}}', langName),
        type: 'danger',
      }))
    )
      return
  ...
  const handleDelete = async (id: number, title: string) => {
    if (
      await confirm({
        title: 'Are you sure?',
        message: t('Delete "{{title}}"?').replace('{{title}}', title),
        type: 'danger',
      })
    ) {
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="mt-1 text-sm text-gray-500">
              {t('Get started by creating a content block')}
            </p>
            <div className="mt-6">
              <Link
                href="/content-blocks/new"
