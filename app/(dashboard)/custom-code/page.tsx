    ...
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-600">Manage custom HTML, CSS, and JavaScript snippets</p>
        </div>
      ...
      <div className="mt-8">
        <div className="mb-6">
          <input type="search" placeholder="Search custom code..." value={search}
        ...
        {isLoading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-2 text-sm text-gray-600">Loading custom code...</p>
          </div>
                  ...
                  {data.data.map((snippet: CustomCode) => (
                    <tr key={snippet.id}>
                      ...
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium">
                        <Link href={`/custom-code/${snippet.id}`} className="text-blue-600 hover:text-blue-900 mr-4">Edit</Link>
                        <button onClick={() => handleDelete(snippet.id, snippet.name)} className="text-red-600 hover:text-red-900" disabled={deleteMutation.isPending}>Delete</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">No custom code</h3>
            <p className="mt-1 text-sm text-gray-500">Get started by adding a code snippet</p>
            <div className="mt-6">
              <Link href="/custom-code/new" className="inline-flex items-center rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Add Code</Link>
            </div>
          </div>
        )}
      </div>
