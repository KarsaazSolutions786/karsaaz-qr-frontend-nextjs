  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{t('New Page')}</h1>
          <p className="mt-2 text-sm text-gray-600">{t('Create a standalone page for your application')}</p>
        </div>
        <Link href="/pages" className="text-sm text-blue-600 hover:text-blue-900">{t('← Back')}</Link>
      </div>

      {createMutation.error && (
        <div className="mb-4 rounded-md bg-red-50 p-4 text-sm text-red-700">{t('Failed to create page. Please check the fields and try again.')}</div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">{t('HTML Content')} <span className="text-red-500">*</span></label>
        ...
        <div>
          ...
          <textarea rows={3} maxLength={160} value={form.metaDescription ?? ''} onChange={(e) => set('metaDescription', e.target.value)}
            ...
            className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none sm:text-sm" />
          <p className="mt-1 text-xs text-gray-500">{(form.metaDescription ?? '').length}/160 {t('characters')}</p>
        </div>
        ...
        <div className="flex items-center justify-end gap-4 border-t pt-4">
          ...
          <button type="submit" disabled={createMutation.isPending}
            ...
            {createMutation.isPending ? t('Creating…') : t('Create Page')}
          </button>
        </div>
      </form>
    </div>
