  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{t('New Translation')}</h1>
          <p className="mt-2 text-sm text-gray-600">{t('Add a new language to your application')}</p>
        </div>
        <Link href="/translations" className="text-sm text-blue-600 hover:text-blue-900">{t('← Back')}</Link>
      </div>

      {createMutation.error && (
        <div className="mb-4 rounded-md bg-red-50 p-4 text-sm text-red-700">{t('Failed to create translation. Please check the fields and try again.')}</div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">{t('Optional flag icon displayed in the language picker')}</p>
        </div>

        <div className="rounded-md border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-600">
          {t('Translation JSON file upload is available after the language is first saved.')}
        </div>
        ...
        <div className="flex items-center justify-end gap-4 border-t pt-4">
          ...
          <button type="submit" disabled={createMutation.isPending || uploading}
            ...
            {createMutation.isPending || uploading ? t('Creating…') : t('Create Translation')}
          </button>
        </div>
      </form>
    </div>
