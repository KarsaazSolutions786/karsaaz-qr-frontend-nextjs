...
export default function NewTranslationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await createMutation.mutateAsync({
      ...form,
      ...(flagFileId ? { flagFileId } : {}),
    } as CreateTranslationRequest)
  }
    ...
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-600">
            {t('Add a new language to your application')}
          </p>
        </div>
      ...
      {createMutation.error && (
        <div className="mb-4 rounded-md bg-red-50 p-4 text-sm text-red-700">
          {t('Failed to create translation. Please check the fields and try again.')}
        </div>
      )}
      ...
      >
        ...
        <div>
          ...
          />
          <p className="mt-1 text-xs text-gray-500">
            {t('Optional flag icon displayed in the language picker')}
          </p>
        </div>
