  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Link href="/translations" className="text-sm text-blue-600 hover:text-blue-900">{t('← Back')}</Link>
      </div>
      ...
      <div className="mb-6 rounded-md border border-blue-200 bg-blue-50 p-4 text-sm">
        ...
        <ul className="space-y-1 text-blue-700">
          ...
          <li>
            ...
            <Link href={`/content-blocks?translation_id=${id}`} className="underline hover:text-blue-900">{t('Content Blocks')}</Link>{' '}
            ·{' '}
            <Link href={`/blog-posts?translation_id=${id}`} className="underline hover:text-blue-900">{t('Blog Posts')}</Link>
          </li>
        </ul>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <div>
          ...
          {isDefault && <p className="mt-1 text-xs text-gray-500">{t('Cannot change the default language name')}</p>}
        </div>
        ...
        <div>
          ...
          <input type="text" disabled={isDefault} value={form.displayName ?? ''} onChange={(e) => set('displayName', e.target.value)}
            ...
            className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none sm:text-sm disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500" />
          <p className="mt-1 text-xs text-gray-500">{t('Used in the language picker shown to end users')}</p>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700">{t('Locale')} <span className="text-red-500">*</span></label>
          ...
          <div className="mt-2 flex gap-6">
            {(['ltr', 'rtl'] as const).map((dir) => (
              <label key={dir} className={`flex cursor-pointer items-center gap-2 ${isDefault ? 'cursor-not-allowed opacity-60' : ''}`}>
                ...
                <span className="text-sm text-gray-700">{dir === 'ltr' ? t('Left to Right (LTR)') : t('Right to Left (RTL)')}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">{t('Upload a new flag to replace the existing one')}</p>
        </div>
      ...
      <div className="mt-8 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-gray-600">{t('Upload a JSON file with translated strings for this language.')}</p>

        {uploadError && (
          <div className="mt-3 rounded-md bg-red-50 p-3 text-sm text-red-700">{uploadError}</div>
        )}
        {uploadSuccess && (
          <div className="mt-3 rounded-md bg-green-50 p-3 text-sm text-green-700">{t('Translation file uploaded successfully.')}</div>
        )}
        ...
        <p className="mt-2 text-xs text-gray-500">{t('Only .json files are accepted. Use the download link above to get the default file structure.')}</p>
      </div>
    </div>
  )
}
