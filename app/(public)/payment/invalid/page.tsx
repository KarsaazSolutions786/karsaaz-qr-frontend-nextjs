...
/**
 * Purpose: Executes PaymentInvalidPage functionality.
...
export default function PaymentInvalidPage() {
  ...
  async function loadMessage() {
    try {
      ...
      setMessage(data?.message || t('Payment verification failed.'))
    } catch {
      setMessage(t('Payment verification failed.'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-8 text-center">
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl p-5 mb-6 text-left">
          <h3 className="text-sm font-semibold text-red-800 mb-2">{t('Possible Issues')}</h3>
          <ul className="text-sm text-red-700 space-y-1.5">
            <li>• {t('Payment processor verification failed')}</li>
            <li>• {t('Session timeout during payment')}</li>
            <li>• {t('Network issue interrupted the process')}</li>
            <li>• {t('Invalid payment response received')}</li>
            <li>• {t('Server error during verification')}</li>
          </ul>
        </div>

        {/* Support Info */}
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6 text-left">
          <h4 className="text-xs font-semibold text-gray-600 uppercase mb-1">{t('When contacting support, please provide:')}</h4>
          <ul className="text-xs text-gray-500 space-y-0.5">
            <li>• {t('Approximate time of payment attempt')}</li>
            <li>• {t('Payment method used')}</li>
            <li>• {t('Error message shown above')}</li>
          </ul>
        </div>
